...
% -------------------------------------------------- 
% 这一段是我自己加的，因为bpass貌似检测不到图片边缘的颗粒，因此首先对图片进行padding，然后再剪裁掉这部分padding
    [m, n] = size(image_array);
    image_array = padarray(image_array, [2*lobject 2*lobject], 'replicate', 'both');
% --------------------------------------------------
...
% 这一段是我自己加的，因为bpass貌似检测不到图片边缘的颗粒，因此首先对图片进行padding，然后再剪裁掉这部分padding
    ...
    res = res(2*lobject+1:2*lobject+m, 2*lobject+1:2*lobject+n);
% --------------------------------------------------